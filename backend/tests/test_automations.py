...
def test_automation_rule_tenant_isolation(client):
...
def test_quiet_hours_skip(client):
    headers = login(client)
...
def test_concurrency_skip(client):
    headers = login(client)
    c = client.post('/automations/rules', headers=headers, json={
...
def test_scheduler_enqueues_workflow_run(client, monkeypatch):
    headers = login(client)
    c = client.post('/automations/rules', headers=headers, json={
