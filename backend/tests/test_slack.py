...
def test_slack_rls_isolation(client, monkeypatch):
...
def test_slack_state_validation(client, monkeypatch):
    headers = login(client)
    client.post('/integrations/slack/connect', headers=headers)
...
def test_slack_post_message_risk_gating_and_allowlist(client, monkeypatch):
    headers = login(client)
    _connect_slack(client, headers, monkeypatch)
...
def test_slack_external_shared_is_high(client, monkeypatch):
    headers = login(client)
    _connect_slack(client, headers, monkeypatch)
...
def test_slack_post_idempotency_dedupe(client, monkeypatch):
    headers = login(client)
    _connect_slack(client, headers, monkeypatch)
    client.put('/tenant/slack-policy', headers=headers, json={'allowed_channel_ids': ['C1'], 'allow_external_shared': False})
...
def test_slack_token_refresh_persists(client, monkeypatch):
    headers = login(client)
    _connect_slack(client, headers, monkeypatch)
    with engine.begin() as conn:
        conn.execute(text("SET app.tenant_id = '10000000-0000-0000-0000-000000000001'"))
