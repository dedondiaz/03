...
def test_workflow_templates_api_contract(client, login):
...
def test_workflow_input_validation(client, login):
    headers = login(client)
    bad = client.post('/workflows/runs', headers=headers, json={'template_id': 'schedule_meeting_v1', 'input': {'title': 'x'}})
    assert bad.status_code == 400


def test_workflow_create_returns_linked_run_id(client, login):
    headers = login(client)
...
def test_workflow_tenant_isolation(client, login):
    headers = login(client)
...
def test_workflow_waiting_approval_propagation(client, monkeypatch, login):
    headers = login(client)
...
def test_workflow_idempotency_no_dup_within_run(client, monkeypatch, login):
    headers = login(client)
