...
def test_automation_policy_owner_only(client, login):
...
def test_automation_sessions_crud(client, login):
    owner = login(client, 'owner@example.com')
...
def test_artifact_access_is_tenant_scoped(client, login):
    owner = login(client, 'owner@example.com')
