...
def test_invite_accept_single_use_and_expiry(client, login):
...
def test_role_permissions_member_cannot_invite(client, login):
    member = login(client, 'member@example.com')
    denied = client.post('/tenant/members/invite', headers=member, json={'email': 'x@example.com', 'role': 'member'})
    assert denied.status_code == 403


def test_policies_hub_aggregate_and_update(client, login):
    owner = login(client, 'owner@example.com')
...
def test_audit_export_csv_redacted_and_scoped(client, login):
    owner = login(client, 'owner@example.com')
...
def test_usage_summary_is_tenant_scoped(client, login):
...
def test_invites_rls_isolation(client, login):
    owner = login(client, 'owner@example.com')
