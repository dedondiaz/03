...
def test_jira_rls_isolation(client, monkeypatch, login):
...
def test_jira_state_validation(client, monkeypatch, login):
    headers = login(client)
    client.post('/integrations/jira/connect', headers=headers)
...
def test_jira_create_issue_gating_and_allowlist(client, monkeypatch, login):
    headers = login(client)
    _connect_jira(client, headers, monkeypatch)
...
def test_jira_transition_always_high(client, monkeypatch, login):
    headers = login(client)
    _connect_jira(client, headers, monkeypatch)
    from app.llm.client import LLMClient
...
def test_jira_idempotency_create_and_comment(client, monkeypatch, login):
    headers = login(client)
    _connect_jira(client, headers, monkeypatch)
    client.put('/tenant/jira-policy', headers=headers, json={'allowed_project_keys':['ENG'], 'allow_write': True})
...
def test_jira_refresh_rotating_token_persists(client, monkeypatch, login):
    headers = login(client)
    _connect_jira(client, headers, monkeypatch)
    client.put('/tenant/jira-policy', headers=headers, json={'allowed_project_keys':['ENG'], 'allow_write': True})
    with engine.begin() as conn:
