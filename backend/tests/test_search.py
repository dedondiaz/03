...
def test_search_tenant_isolation_notion_docs(client):
...
def test_search_partial_connectivity_warnings(client, monkeypatch):
    headers = login(client)
...
def test_unified_search_tool_redaction_persistence(client, monkeypatch):
    headers = login(client)
...
def test_slack_adapter_policy_enforcement(client, monkeypatch):
    headers = login(client)
