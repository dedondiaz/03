...
def test_oauth_state_validation(client, monkeypatch, login):
...
def test_rls_isolation_google_credentials_and_policy(client, monkeypatch, login):
    headers = login(client)
...
def test_gmail_send_high_risk_gate_and_approval(client, monkeypatch, login):
    headers = login(client)
...
def test_token_refresh_behavior(client, monkeypatch, login):
    headers = login(client)
    monkeypatch.setattr(oauth, 'exchange_code', lambda code, verifier: {
...
def test_gmail_send_idempotency_same_run(client, monkeypatch, login):
    headers = login(client)
