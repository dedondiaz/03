...
def test_rls_isolation_tenant_calendar_settings(client):
...
def test_calendar_cancel_requires_approval(client, monkeypatch):
    headers = login(client)
...
def test_calendar_create_external_is_high_and_blocked_without_approval(client, monkeypatch):
    headers = login(client)
    _connect_google(client, headers, monkeypatch)
...
def test_calendar_refresh_persists_token(client, monkeypatch):
    headers = login(client)
    _connect_google(client, headers, monkeypatch)
...
def test_calendar_idempotency_prevents_duplicate_create_and_cancel(client, monkeypatch):
    headers = login(client)
    _connect_google(client, headers, monkeypatch)
